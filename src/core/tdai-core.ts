...
export class TdaiCore {
  ...
  private wirePipelineRunners(): void {
    ...
    // Determine whether to use the host LLM runner factory for extraction.
    // A non-openclaw host (dsh) always uses the host adapter's runner factory.
    const useHostRunner = this.cfg.llm.enabled || this.hostAdapter.hostType !== "openclaw";

    const openclawConfig = undefined;

    // The dsh host always resolves runners through the host adapter factory.
    let runnerFactory = this.runnerFactory;

    const l1LlmRunner = useHostRunner
      ? runnerFactory.createRunner({ enableTools: false })
      : undefined;
    const l2l3LlmRunner = useHostRunner
