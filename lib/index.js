...
async function readConversationRecords(sessionKey, baseDir, logger) {
  ...
  const targetFiles = entries.filter((name2) => dateFilePattern.test(name2)).sort();
  if (targetFiles.length === 0) {
    return [];
  }
  const records = [];
...
var TcvdbApiError = class extends Error {
  apiCode;
  constructor(path19, code, msg) {
    super(`VectorDB ${path19}: code=${code}, msg=${msg}`);
...
var TcvdbClient = class {
   ...
   */
  async request(path19, body) {
    let lastError;
    const t0 = performance.now();
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      const tAttempt = performance.now();
      try {
        this.logger?.debug?.(`${TAG11} \u2192 ${path19} attempt=${attempt} body=${JSON.stringify(body).slice(0, 500)}`);
        const { statusCode, body: respBody } = await undiciRequest(`${this.baseUrl}${path19}`, {
          method: "POST",
          headers: {
        ...
        this.logger?.debug?.(`${TAG11} \u2190 ${path19} status=${statusCode} code=${json.code} attemptMs=${attemptMs} attempt=${attempt}`);
        if (json.code !== 0) {
          const err = new TcvdbApiError(path19, json.code, json.msg);
          if (statusCode !== void 0 && statusCode >= 400 && statusCode < 500) throw err;
          lastError = err;
          continue;
        }
        const totalMs2 = Math.round(performance.now() - t0);
        this.logger?.info(`${TAG11} ${path19} ${totalMs2}ms${attempt > 0 ? ` (${attempt + 1} attempts)` : ""}`);
        return json;
      } catch (err) {
        const attemptMs = Math.round(performance.now() - tAttempt);
        ...
        if (attempt < MAX_RETRIES) {
          ...
          this.logger?.debug?.(`${TAG11} ${path19} retry ${attempt + 1}/${MAX_RETRIES} in ${delay}ms (lastAttemptMs=${attemptMs}, error=${lastError.message})`);
          await new Promise((r) => setTimeout(r, delay));
        }
      }
    }
    const totalMs = Math.round(performance.now() - t0);
    this.logger?.debug?.(`${TAG11} \u2717 ${path19} totalMs=${totalMs} attempts=${MAX_RETRIES + 1} error=${lastError?.message}`);
    throw lastError ?? new Error(`${TAG11} ${path19} failed after retries`);
  }
  // ── Database operations ─────────────────────────────────
  async createDatabase(dbName) {
    const name2 = dbName ?? this.database;
    const listResp = await this.request("/database/list", {});
    const exists = (listResp.databases ?? []).includes(name2);
    if (exists) {
      this.logger?.debug?.(`${TAG11} Database already exists: ${name2}`);
      return false;
    }
    await this.request("/database/create", { database: name2 });
    this.logger?.info(`${TAG11} Database created: ${name2}`);
    return true;
  }
  // ── Collection operations ───────────────────────────────
  async createCollection(params) {
    const name2 = String(params.collection ?? "");
    try {
      await this.describeCollection(name2);
      this.logger?.debug?.(`${TAG11} Collection already exists: ${name2}`);
      return;
    } catch (err) {
    ...
    try {
      ...
      this.logger?.info(`${TAG11} Collection created: ${name2}`);
    } catch (err) {
      if (err instanceof TcvdbApiError && err.apiCode === 15202) {
        this.logger?.debug?.(`${TAG11} Collection already exists (race): ${name2}`);
        return;
      }
      throw err;
    }
...
async function pruneOldEntries(dir, maxKeep, kind) {
  ...
  for (const name2 of toRemove) {
    try {
      if (kind === "file") {
        await fs8.unlink(path9.join(dir, name2));
      } else {
        await fs8.rm(path9.join(dir, name2), { recursive: true, force: true });
      }
    } catch {
    }
  }
...
function normalizeSceneFilename(name2) {
  if (!name2) return "scene.md";
...
// src/index.ts
var TAG23 = "[memory-tdai]";
function envStr(name2) {
  const v = process.env[name2];
  return v !== void 0 && v.length > 0 ? v : void 0;
}
function envBool(name2) {
  const v = process.env[name2];
...
function buildConfig(rawConfig) {
  ...
  const overrides = {};
  if (envStr("DSH_MEMORY_DATA_DIR") !== void 0) overrides.dataDir = envStr("DSH_MEMORY_DATA_DIR");
  if (envBool("DSH_MEMORY_CAPTURE_ENABLED") !== void 0) overrides.capture = { enabled: envBool("DSH_MEMORY_CAPTURE_ENABLED") };
  if (envBool("DSH_MEMORY_RECALL_ENABLED") !== void 0) overrides.recall = { enabled: envBool("DSH_MEMORY_RECALL_ENABLED") };
  if (envBool("DSH_MEMORY_EXTRACTION_ENABLED") !== void 0) overrides.extraction = { enabled: envBool("DSH_MEMORY_EXTRACTION_ENABLED") };
  return parseConfig({ ...base, ...overrides });
}
function resolveDataDir(rawConfig) {
  const rc = rawConfig && typeof rawConfig === "object" ? rawConfig : {};
  if (rc.dataDir && typeof rc.dataDir === "string" && rc.dataDir.length > 0) return rc.dataDir;
  const dshHome = process.env.DSH_HOME ?? path18.join(os.homedir(), ".dsh");
  return path18.join(dshHome, "memory-tdai");
}
function resolveRoute(rawConfig, agents) {
  const rc = rawConfig && typeof rawConfig === "object" ? rawConfig : {};
...
function messagesText(messages) {
  return messages.map(
    (m) => (m.content ?? []).filter((b) => b.type === "text" && typeof b.text === "string").map((b) => b.text).join("\n")
  ).filter((t) => t.length > 0).join("\n");
}
function blocksText(blocks) {
  return (blocks ?? []).filter((b) => b.type === "text" && typeof b.text === "string").map((b) => b.text).join("\n");
}
function loggerAdapter(ctx) {
  const l = ctx.logger;
  return {
    info: (m) => l?.info?.(m),
    warn: (m) => l?.warn?.(m),
...
var name = "memory-tdai";
var inject = ["llm", "tools", "systemPrompt"];
function apply(ctx, config = {}) {
  ...
  logger.info(
    `${TAG23} mounting dsh-memory-tdai: dataDir=${dataDir}, route=${route.provider}/${route.model}, capture=${cfg.capture.enabled}, recall=${cfg.recall.enabled}, extraction=${cfg.extraction.enabled}`
  );
  ...
  const coreReady = core.initialize().catch((err) => {
    logger.error(`${TAG23} core init failed: ${err instanceof Error ? err.message : String(err)}`);
  });
  const recallCache = /* @__PURE__ */ new Map();
  const turnPromptCache = /* @__PURE__ */ new Map();
  const sessionMessageCount = /* @__PURE__ */ new Map();
  const tools = ctx.tools;
  tools.register({
    name: "memory_search",
    description: "Search the user's long-term memories (four-layer memory store: persona, episodic events, instructions). Use this to recall the user's preferences, past events, rules, or context from previous conversations. Returns ranked memory records. Prefer this over conversation_search for structured facts.",
    parameters: {
      type: "object",
      properties: {
    ...
    async execute(args) {
      ...
      const typeFilter = typeof args.type === "string" ? args.type : void 0;
      const sceneFilter = typeof args.scene === "string" ? args.scene : void 0;
      try {
  ...
  tools.register({
    ...
    description: "Search past conversation history (raw dialogue records). Use when memory_search does not have the answer or when you need exact words, dialogue context, or specific past messages. Returns ranked individual messages.",
    parameters: {
      type: "object",
      properties: {
    ...
    async execute(args) {
      ...
      const query = String(args.query ?? "");
      const limit = Math.min(Math.max(Number(args.limit) || 5, 1), 20);
      const sessionKeyFilter = typeof args.session_key === "string" ? args.session_key : void 0;
      try {
  ...
  logger.info(`${TAG23} registered memory_search / conversation_search tools`);
  ctx.on("agent/pre-step", async (payload, next) => {
    const decision = await next();
    if (decision === void 0 || decision.kind !== "enter") return decision;
    const { agent } = payload;
    try {
      const sessionKey = String(agent.session.id);
      const userText = messagesText(payload.messages ?? []);
      ...
      logger.warn(`${TAG23} auto-recall failed (non-blocking): ${err instanceof Error ? err.message : String(err)}`);
    }
    return decision;
  });
  ...
  ctx.on("agent/turn-stopping", async (payload) => {
    ...
    try {
      ...
      logger.warn(`${TAG23} auto-capture failed (non-blocking): ${err instanceof Error ? err.message : String(err)}`);
    }
  });
  ctx.effect(() => () => {
    core.destroy().catch((err) => {
      logger.warn(`${TAG23} destroy failed: ${err instanceof Error ? err.message : String(err)}`);
    });
  });
  logger.info(`${TAG23} dsh-memory-tdai mounted`);
}
export {
  apply,
  inject,
